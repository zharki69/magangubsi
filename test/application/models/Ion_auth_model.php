...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Ben Edmunds
	 */
	public function users($groups = NULL, $kondisi = NULL, $order_by = NULL)
	{
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description, ' . $this->tables['groups'] . '.link')
	...
	public function update_last_login($id)
	{
		...
		$data = array(
			'ip_address' => $this->input->ip_address(),
			'last_login' => time(),
			'last_user_agent' => $this->agent->agent_string(),
		);
